...
def derive_boards(tm, life_refs, candidates):
    ...
    derived = {}
    for cid in range(1, 7):
        t = next((x for x in tm.triggers if (x.name or '') == f'{cid}毒'), None)
        if t is None:
            raise BuildError(f'缺前置：找不到原觸發「{cid}毒」（牌ref推導用）')
        ...
        if len(hits) != 1:
            raise BuildError(f'缺前置：「{cid}毒」狀態牌寫入不是恰一個：{hits}')
...
git commit -m "feat(合併): s51 裁決表載入與牌ref推導驗證（TDD）"
```
...
Run: `../../.venv/bin/python -m pytest tests/test_s51.py -q`
Expected: 前 6 個 pass、新 6 個 FAIL（caption_calls 空／未 raise）。
...
```python
        ...
        mount_refs = {int(k): int(v)
                      for k, v in (ctx.spec.params.get('revive') or {})['mount_refs'].items()}
...
def test_zero_condition_writer_reported(f):
    from steps.s51_status_caption import StatusCaptionStep
    ...
    ctx, _ = make_ctx(f, extra_trigs=[child, parent])
    changes = StatusCaptionStep().apply(ctx)
    reports = [c for c in changes if c.kind == 'report' and '1酒3' in c.target]
    assert len(reports) == 1 and '1酒2' in reports[0].new
...
Run: `../../.venv/bin/python -m pytest tests/test_s51.py -q`
Expected: hooks／report 相關 4 個 FAIL（`test_noop_without_params` 可能已過）。

- [ ] **Step 3: 補 apply 尾段與 test_guide**

(a) 在 apply 的 `for t, cid, raw in writes:` 迴圈內、`changes.append(Change(...))` 之後**同層**追加零條件報告：

```python
            if not t.conditions:          # 報告項：零條件寫入器（spec §四）
                ...
                changes.append(Change(self.id, 'report', f'T{t.trigger_id}「{t.name}」',
                                      '—', '',
                                      f'零條件寫入器；父觸發：{"、".join(parents) or "無"}',
                                      '騎預置馬期間可能顯示缺口，複核用'))
```
...
```python
    def test_guide(self, changes):
        ...
        return (f'狀態頭頂字幕：附掛＋清字共 {n} 效果、報告項 {rep}（零條件寫入器）。\n'
...
grep -c '零條件寫入器' logs/51_狀態字幕.tsv
```
Expected: `768`／`72`／`108`／數十（零條件寫入器數，記下數字供 review）。

- [ ] **Step 4: Commit（建置產物依 repo 慣例一併入版控）**
