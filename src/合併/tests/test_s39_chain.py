...
MOUNT = {1: dict(mount_ref=26109, flag_cell=(221, 239), rebirth_cell=(230, 239),
         ...
         2: dict(mount_ref=26186, flag_cell=(221, 238), rebirth_cell=(230, 238),
                 final_cell=(232, 238))}
LIFE_VARS = {1: 21, 2: 22}          # 命數變數（取代命旗，2026-09-01）
...
def test_timer_revives_next_life(f):
    ...
    # 命數遞移 1→2（取代舊制的除舊旗/立新旗）
    assert [(kw['variable'], kw['quantity'], kw['operation'])
            for kw in _effs(t1, 'change_variable')] == [(21, 2, 1)]
    assert not any(kw.get('object_list_unit_id') == 720 for kw in removes)
...
def test_final_creates_flag_and_revealer(f):
    ...
    # 命數歸零 → locref 家族熄火
    assert [(kw['variable'], kw['quantity'], kw['operation'])
            for kw in _effs(fin, 'change_variable')] == [(21, 0, 1)]
...
def test_horse_watch_per_life(f):
    ...
    assert [(kw['variable'], kw['quantity'], kw['operation'])
            for kw in _effs(hwf, 'change_variable')] == [(21, 0, 1)]
...
def test_selection_trigger(f):
    ...
    assert [(kw['variable'], kw['quantity'], kw['operation'])
            for kw in _effs(sel, 'change_variable')] == [(21, 1, 1)]
    assert not any(kw.get('object_list_unit_id') == 720
