# -*- coding: utf-8 -*-
"""渡船特徵斷言（feature invariants）。由 s91 在建置中呼叫（違規即中止），
也供 tools/verify_ferry.py 當 CLI 對已完成的產物跑一遍。

2026-08-31 起因：渡船這套機制連續三次「建置全綠、進遊戲才炸」。這些斷言就是把
「接線／變體／座標／中和」的預期寫死成可自動比對的形狀，而不是靠人記得去看。
回傳 [(ok, 說明)]。"""
